(async () => {
  ...
  const LEGACY_EXTRA_FOLDERS = ['Relíquias', 'Outros'];
  ...
  const confirmed = await Dialog.confirm({
    title: 'Limpar e Reimportar Compêndio',
    content: '<p>Isso vai apagar todos os Actors de <strong>ordem-bestiario.ameacas</strong> antes de importar o novo JSON. Deseja continuar?</p>',
    yes: () => true,
    no: () => false,
    defaultYes: false
  });
  ...
  if (!confirmed) {
    ui.notifications?.warn('Operação cancelada pelo usuário.');
    return;
  }

  const rawJson = await Dialog.prompt({
    title: 'Colar JSON para Reimportação',
    content: `
      <form>
  ...
  if (!rawJson) {
    ui.notifications?.warn('Reimportação cancelada: nenhum JSON informado.');
    return;
  }
  ...
  const resolveFolderName = (resolvedElement) => {
    return FOLDER_BY_ELEMENT[resolvedElement] ?? 'Conhecimento';
  };
  ...
  const cleanupLegacyFolders = async (packCollection) => {
    const currentFolders = game.folders.filter((f) => f.pack === packCollection && f.type === 'Actor');
    const legacyFolders = currentFolders.filter((f) => LEGACY_EXTRA_FOLDERS.includes(f.name));
    if (!legacyFolders.length) return;
    warn(
      `Pastas legadas detectadas no compêndio (${legacyFolders.map((f) => f.name).join(', ')}). ` +
      'A macro não vai deletá-las automaticamente para evitar erro de compêndio bloqueado. Remova manualmente se quiser.'
    );
  };
  ...
  let skipped = 0;
  const errors = [];
  const folderCounters = Object.fromEntries(REQUIRED_FOLDERS.map((name) => [name, 0]));
  const importLogRows = [];
  ...
  try {
    ...
    if (docs.length) {
      ...
      deleted = ids.length;
      log(`Removidos ${deleted} actor(es) existentes do compêndio.`);
    }

    await cleanupLegacyFolders(pack.collection);
    ...
    for (const sourceActor of parsed) {
      ...
      try {
        ...
        if (!resolvedFolder) {
          throw new Error(`Pasta "${resolvedFolderName}" não disponível no compêndio.`);
        }
        if (!FOLDER_BY_ELEMENT[resolvedElement]) {
          warn(`Actor sem elemento identificado: ${name}`);
        }
        ...
        imported += 1;
        folderCounters[resolvedFolderName] = (folderCounters[resolvedFolderName] ?? 0) + 1;
        importLogRows.push({
          name,
  ...
  const usedFolders = [...new Set(importLogRows.map((row) => row.folder))];
  const invalidUsedFolders = usedFolders.filter((name) => !REQUIRED_FOLDERS.includes(name));
  if (invalidUsedFolders.length) {
    error('Pastas inválidas detectadas na importação:', invalidUsedFolders);
  }

  const byFolderSummary = REQUIRED_FOLDERS.map((name) => `${name}: ${folderCounters[name] ?? 0}`).join(', ');
  const summary = `Reimportação concluída: ${deleted} removido(s), ${imported} importado(s), ${skipped} pulado(s), ${failed} erro(s). Por pasta -> ${byFolderSummary}`;
  ui.notifications?.info(summary);
  log(summary);
  if (errors.length) {
    warn('Erros de importação:', errors);
  }
})();
